...
<body>
          ...
          <div class="card-header">
            <div class="row align-items-center">
              ...
              <div class="col-md-6 text-md-end mt-3 mt-md-0">
                <h4 class="mb-1 text-dark">Invoice #{{ $invoice->customer_id }}</h4>
                <p class="mb-1"><i class="far fa-calendar icon-primary me-1"></i> Tanggal: {{ date('d M Y') }}</p>
                <p class="mb-2"><i class="far fa-clock icon-primary me-1"></i> Jatuh Tempo:
              ...
              <div class="col-md-2 mt-4">
                <button class="btn btn-primary btn-xs fw-bold w-100" data-bs-toggle="modal"
                  data-bs-target="#invoiceHistoryModal">
                  <i class="fas fa-history me-2"></i>Lihat Riwayat Tagihan
                </button>
              </div>
            </div>
          </div>
              ...
              <div class="col-md-6 mb-4">
                ...
                <div class="ps-3">
                  ...
                  <p class="mb-1 text-muted"><i
                      class="fas fa-envelope icon-secondary me-1"></i>{{ $invoice->customer->email }}</p>
                  <p class="mb-0 text-muted"><i
                      class="fas fa-phone icon-secondary me-1"></i>{{ $invoice->customer->no_hp }}</p>
                </div>
              </div>
              <div class="col-md-6 text-md-end">
                ...
                <div class="ps-3">
                  ...
                  <p class="mb-0 text-muted">Periode:
                    {{ \Carbon\Carbon::parse($invoice->jatuh_tempo)->locale('id')->isoFormat('MMMM') }}
                  </p>
                </div>
              </div>
            </div>
          ...
          <div class="sticky-top" style="top: 20px;">
            ...
            <div class="payment-card fade-in">
              ...
              <div class="card-body">
                ...
                <form action="{{ route('tripay.payment', encrypt($invoice->customer_id)) }}?invoice_id={{ $invoice->id }}"
    ...
    <div class="modal-dialog modal-xl modal-dialog-scrollable">
      <div class="modal-content">
        <div class="modal-header bg-primary text-white">
          <h5 class="modal-title" id="invoiceHistoryModalLabel">
            <i class="fas fa-history me-2"></i>Riwayat Tagihan
          </h5>
        ...
        <div class="modal-body">
          ...
          <div class="row mb-3">
            <div class="col-md-4">
              ...
              <select class="form-select" id="yearFilter" onchange="filterInvoicesByYear(this.value)">
                @php
                  $currentYear = \Carbon\Carbon::now()->year;
                  $startYear = 2020; // Tahun mulai bisnis
                @endphp
            ...
            <!-- <div class="col-md-8 d-flex align-items-end mt-3">
              <div class="alert alert-warning mb-0 w-100">
                <p class="fw-bold"><i class="fa-solid fa-info-circle me-2"></i>Akumulasi Total Tagihan Tahun
                  {{ date('Y') }} :
                  Rp
                  {{ number_format($invoiceAll->where('status_id', '!=', 8)->sum('tagihan'), 0, ',', '.') }}
                </p>
              </div>
            </div> -->
          </div>
          ...
          <div class="table-responsive">
            <table class="table table-hover invoice-table" id="invoiceHistoryTable">
              <thead class="table-dark">
                <tr>
              ...
              <tbody id="invoiceHistoryBody">
                @foreach($invoiceAll->sortByDesc('jatuh_tempo') as $inv)
                  <tr data-year="{{ \Carbon\Carbon::parse($inv->jatuh_tempo)->year }}"
                    class="{{ $inv->id == $invoice->id ? 'table-primary' : '' }}">
                    <td class="text-center">
                      @if($inv->status_id == 8)
                        <!-- Button Print -->
                        <a href="/print-kwitansi/{{ encrypt($inv->id) }}" class="btn btn-sm btn-outline-success"
                          target="_blank">
                          <i class="fas fa-print me-1"></i> Print
                        </a>
                      @else
                        <!-- Button Bayar -->
                        <a href="/payment/invoice/{{ encrypt($inv->customer_id) }}?invoice_id={{ $inv->id }}"
                    ...
                    <td>
                      <div class="d-flex align-items-center">
                        <i class="fas fa-calendar-alt icon-primary me-2"></i>
                        <span
                          class="fw-semibold">{{ \Carbon\Carbon::parse($inv->jatuh_tempo)->locale('id')->isoFormat('MMMM YYYY') }}</span>
                        @if($inv->id == $invoice->id)
                          <span class="badge bg-info ms-2">
                            <i class="fas fa-eye me-1"></i>Sedang Ditampilkan
                          </span>
                        @endif
                      </div>
                    </td>
                    <td>{{ $inv->paket->nama_paket ?? '-' }}</td>
                    <td class="text-end">Rp {{ number_format($inv->tagihan, 0, ',', '.') }}</td>
                    <td class="text-end">Rp {{ number_format($inv->tambahan ?? 0, 0, ',', '.') }}</td>
                    <td class="text-end">Rp {{ number_format($inv->tunggakan ?? 0, 0, ',', '.') }}</td>
                    <td class="text-end fw-bold">
                      Rp
                      {{ number_format($inv->tagihan + ($inv->tambahan ?? 0) + ($inv->tunggakan ?? 0) - ($inv->saldo ?? 0), 0, ',', '.') }}
                    </td>
                    <td class="text-center">
                      @if($inv->status_id == 8)
          ...
          <div id="emptyState" class="text-center py-5 d-none">
            <i class="fas fa-inbox icon-secondary mb-3" style="font-size: 3rem; opacity: 0.3;"></i>
            <p class="text-muted">Tidak ada tagihan untuk tahun yang dipilih</p>
          </div>
        </div>
  ...
  <footer class="bg-dark text-white py-4 mt-5 no-print">
    <div class="container text-center">
      ...
      made with <i class="fas fa-heart text-danger mx-1"></i> by <a href="https://www.instagram.com/riskimispanda_/"
        target="_blank" class="text-white">Panda 🐼</a>
    </div>
  </footer>
</body>
<!-- Bootstrap JS -->
...
<script>
  ...
  */
  ...
  function filterInvoicesByYear(year) {
    const rows = document.querySelectorAll('#invoiceHistoryBody tr');
    ...
    rows.forEach(row => {
      ...
      if (rowYear == year) {
        ...
        // Calculate unpaid total for selected year
        const statusBadge = row.querySelector('.badge');
        if (statusBadge && !statusBadge.classList.contains('bg-success')) {
          const tagihanText = row.querySelectorAll('td')[2].textContent;
          const tagihan = parseInt(tagihanText.replace(/[^0-9]/g, ''));
          totalUnpaid += tagihan;
        }
      } else {
  ...
  /**
   * Select invoice to pay - close modal and scroll to payment form
   * @param {number} invoiceId - Invoice ID to pay
   * @param {string} encryptedCustomerId - Encrypted customer ID
   */
  function selectInvoiceToPay(invoiceId, encryptedCustomerId) {
    // Close modal
    const modal = bootstrap.Modal.getInstance(document.getElementById('invoiceHistoryModal'));
    if (modal) {
      modal.hide();
    }
    ...
    notification.innerHTML = `
      ...
      <strong>Silakan pilih metode pembayaran</strong> untuk invoice yang dipilih
    ...
    if (paymentCard) {
      paymentCard.classList.add('border', 'border-primary', 'border-3');
      setTimeout(() => {
        paymentCard.classList.remove('border', 'border-primary', 'border-3');
      }, 3000);
    }
  }
