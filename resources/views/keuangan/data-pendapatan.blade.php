...
@section('content')
        ...
        <div class="revenue-card p-4">
          <div class="d-flex justify-content-between align-items-center">
            <div>
              ...
              <h5 id="monthlyRevenueValue" class="fw-bold text-dark mb-0">Rp
      ...
      <div class="revenue-card p-4">
        <div class="d-flex justify-content-between align-items-center">
          <div>
            ...
            <h5 id="pendingRevenueValue" class="fw-bold text-dark mb-0">Rp
              {{ number_format($pendingRevenue ?? 0, 0, ',', '.') }}
            </h5>
          </div>
  ...
  <div class="table-card mb-5" id="invoiceTable">
    <div class="p-6 border-bottom">
      ...
      <small class="fw-semibold badge bg-danger bg-opacity-10 text-danger mt-3 mb-3">Estimasi Pendapatan Bulan
        {{ date('M') }} : Rp {{ number_format($totalEstimasi ?? 0, 0, ',', '.') }}</small>
    </div>

    <div class="d-flex flex-column flex-sm-row justify-content-between align-items-center gap-3 p-4 border-bottom">
      <div class="d-flex flex-column flex-sm-row gap-2" id="tutorial-wa-container">
        ...
        <button type="button" class="btn btn-sm btn-primary" id="btn-tutorial-wa" data-bs-toggle="tooltip"
          data-bs-placement="bottom" title="Panduan Kirim Whatsapp">
          <i class="bx bx-help-circle me-1"></i> Panduan Kirim Whatsapp
        </button>
      </div>
          ...
          <tbody id="tableBody" style="font-size: 14px;">
            @forelse($invoices ?? [] as $index => $invoice)
              <tr class="@if($invoice->customer->trashed()) bg-danger bg-opacity-10 @endif">
                <td>
                  <button class="btn btn-sm btn-link p-0 me-2 collapse-toggle" data-bs-toggle="collapse"
                    data-bs-target="#detail-{{ $invoice->id }}" aria-expanded="false" style="text-decoration: none;">
                    <i class="bx bx-plus-circle text-primary"></i>
                  </button>
                </td>
                <td class="fw-medium">
                  <span>+ {{ $invoices->firstItem() + $index }}</span>
                </td>
                <td>
                  <div>
                      ...
                      <div class="col-md-6">
                        ...
                        <div class="d-flex align-items-center gap-2">
                          <input type="text" class="form-control form-control-sm"
                            value="{{ url('/payment/invoice/' . encrypt($invoice->customer_id)) }}"
                            id="paymentLink{{ $invoice->id }}" readonly>
                          <button class="btn btn-sm btn-primary" onclick="copyToClipboard('paymentLink{{ $invoice->id }}')"
    ...
    <div class="p-4 border-top">
      <div class="d-flex flex-column flex-sm-row justify-content-between align-items-center gap-3">
        <div class="text-muted small">
          Menampilkan data {{ $invoices->firstItem() ?? 1 }} sampai
          {{ $invoices->lastItem() ?? ($invoices->count() ?? 0) }} dari
          {{ number_format($invoices->total() ?? $invoices->count() ?? 0, 0, ',', '.') }} data
        </div>
        <div>
          <nav aria-label="Page navigation">
            <ul class="pagination pagination-sm justify-content-center mb-0">
              ...
              @else
                <li class="page-item">
                  <a class="page-link ajax-pagination" href="#" data-page="{{ $invoices->currentPage() - 1 }}">‹
                    Previous</a>
                </li>
              @endif
              ...
              @if ($end < $invoices->lastPage())
                ...
                @endif
                <li class="page-item">
                  <a class="page-link ajax-pagination" href="#"
                    data-page="{{ $invoices->lastPage() }}">{{ $invoices->lastPage() }}</a>
                </li>
              @endif
  ...
  @foreach ($invoices as $invoice)
    <div class="modal fade" id="konfirmasiPembayaran{{ $invoice->id }}" tabindex="-1" style="display: none;"
      aria-hidden="true">
            ...
            <div class="modal-body">
              ...
              <div class="row">
                <div class="col mb-4">
                  ...
                  <input type="date" class="form-control"
                    value="{{ \Carbon\Carbon::parse($invoice->jatuh_tempo)->format('Y-m-d') }}" readonly>
                </div>
              </div>
              ...
              <div class="row">
                <div class="col mb-4 col-lg-4">
                  ...
                  <div class="form-check">
                    <input type="checkbox" class="form-check-input pilihan" name="bayar[]" value="tagihan"
                      data-amount="{{ $invoice->tagihan ?? 0 }}" data-id="{{ $invoice->id }}">
                ...
                <div class="col mb-4 col-lg-4">
                  ...
                  <div class="form-check">
                    <input type="checkbox" class="form-check-input pilihan" name="bayar[]" value="tambahan"
                      data-amount="{{ $invoice->tambahan ?? 0 }}" data-id="{{ $invoice->id }}">
                    <label class="form-check-label">
                ...
                <div class="col mb-4 col-lg-4">
                  ...
                  <div class="form-check">
                    <input type="checkbox" class="form-check-input pilihan" name="bayar[]" value="tunggakan"
                      data-amount="{{ $invoice->tunggakan ?? 0 }}" data-id="{{ $invoice->id }}">
                    <label class="form-check-label">
                ...
                <div class="col mb-4 col-lg-4">
                  ...
                  <div class="form-check">
                    <input type="checkbox" class="form-check-input pilihan" name="saldo" value="{{ $invoice->saldo }}"
                      data-amount="{{ $invoice->saldo ?? 0 }}" data-id="{{ $invoice->id }}" data-type="saldo">
                    <label class="form-check-label">
              ...
              <div class="row g-2">
                <div class="col mb-4 col-lg-6">
                  ...
                  <input type="text" class="form-control" id="revenueAmount{{ $invoice->id }}" name="revenueAmount"
                    oninput="formatRupiah(this, {{ $invoice->id }})" placeholder="Masukkan jumlah bayar" required>
              ...
              <div class="row">
                <div class="col mb-4 col-lg-12">
                  ...
                  <textarea class="form-control" id="keteranganPembayaran" name="keterangan" rows="4"
          ...
          <div class="modal-body mb-3">
            <div class="row gy-3 gx-4">
              ...
              <div class="col-md-6">
                ...
                <div class="d-flex align-items-center gap-2 fs-6">
                  <i class="bx bx-plus-circle text-primary"></i>
                  <span class="text-dark" id="additionalAmount">Rp
              ...
              .then(data => {
                if (data.status === 'success') {
                  ...
                  if (data.offset_next < total && data.processed > 0) {
                    // Menambahkan jeda 1 detik untuk melindungi kestabilan server (CPU Spikes)
                    setTimeout(() => {
  ...
  <script>
    ...
    // Toggle icon for collapse (for both initial load and AJAX loaded rows)
    $(document).on('show.bs.collapse', '[id^="detail-"]', function () {
      const button = $(this).prev('tr').find('.collapse-toggle');
      button.find('i').removeClass('bx-plus-circle').addClass('bx-minus-circle');
    });

    $(document).on('hide.bs.collapse', '[id^="detail-"]', function () {
      const button = $(this).prev('tr').find('.collapse-toggle');
      button.find('i').removeClass('bx-minus-circle').addClass('bx-plus-circle');
    });
  ...
  <script>
    document.addEventListener('DOMContentLoaded', function () {
      // driver.js tutorial logic for WhatsApp broadcast
      if (typeof window.driver !== 'undefined') {
        const driver = window.driver.js.driver;
        const driverObjWA = driver({
          showProgress: true,
          nextBtnText: 'Lanjut &rarr;',
          prevBtnText: '&larr; Kembali',
          doneBtnText: 'Selesai',
          steps: [
            { element: '#tutorial-wa-container', popover: { title: 'Fitur Kirim WhatsApp', description: 'Gunakan fitur ini untuk mengirimkan pengingat tagihan/invoice secara global ke semua pelanggan yang berstatus "Belum Bayar" pada periode terpilih.', side: "bottom", align: 'start' } },
            { element: '#btnKirimGlobal', popover: { title: 'Kirim Pesan', description: 'Klik tombol ini. Harap beri waktu pada sistem karena ia akan mengirimkan pesan satu per satu melalui server WhatsApp (Mekari Qontak). Jangan menutup halaman sebelum proses selesai.', side: "bottom", align: 'start' } },
            { element: '#filterForm', popover: { title: 'Filter Bulan/Tahun', description: 'Pesan WhatsApp hanya akan dikirimkan kepada tagihan pada Bulan dan Tahun yang sedang Anda filter di atas ini.', side: "bottom", align: 'start' } },
          ]
        });

        document.getElementById('btn-tutorial-wa').addEventListener('click', function () {
          driverObjWA.drive();
        });
      }
    });
