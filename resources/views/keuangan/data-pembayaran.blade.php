...
@section('content')
    ...
    <div class="col-12 col-sm-6 col-lg-3">
      <div class="payment-card p-4">
        ...
        <div class="d-flex justify-content-between align-items-center mt-3">
          <small class="fw-medium text-muted">Jumlah Pelanggan: {{$paymentNow}}</small>
          <a href="/pembayaran/daily" data-bs-toggle="tooltip" data-bs-placement="top"
            title="Lihat detail pembayaran hari ini">
  ...
  <div class="payment-method-section">
    <h6 class="fw-semibold mb-3">
      ...
      @else
        <small class="text-muted fw-normal">(Semua Bulan)</small>
      @endif
    </h6>
        ...
        <div class="payment-card method-card cash-card p-4">
          <div class="d-flex justify-content-between align-items-start">
            <div class="flex-grow-1">
              ...
              <div class="stat-number text-success mb-4">Rp
                {{ number_format($cashPayments ?? 0, 0, ',', '.') }}
              </div>
        ...
        <div class="payment-card method-card transfer-card p-4">
          <div class="d-flex justify-content-between align-items-start">
            <div class="flex-grow-1">
              ...
              <div class="stat-number text-primary mb-3">Rp
                {{ number_format($transferPayments ?? 0, 0, ',', '.') }}
              </div>
              <div class="stat-sublabel">Total transaksi</div>
            </div>
            <div class="stat-icon bg-danger bg-opacity-10 text-danger">
        ...
        <div class="payment-card method-card ewallet-card p-4">
          <div class="d-flex justify-content-between align-items-start">
            <div class="flex-grow-1">
              ...
              <div class="stat-number text-warning mb-3">Rp
                {{ number_format($ewalletPayments ?? 0, 0, ',', '.') }}
              </div>
              <div class="stat-sublabel">Total transaksi</div>
            </div>
            <div class="stat-icon bg-warning bg-opacity-10 text-warning">
        ...
        <div class="col-12 col-md-6 col-lg-4">
          ...
          <select id="monthFilter" name="month" class="form-select" title="Pilih Bulan">
            ...
            @foreach ($months as $value => $name)
              <option value="{{ $value }}" {{ ($month ?? date('n')) == $value ? 'selected' : '' }}>
                {{ $name }}
              </option>
            @endforeach
          </select>
      ...
      <div class="d-flex flex-column flex-sm-row gap-2">
        <div class="dropdown">
          ...
          <ul class="dropdown-menu">
            ...
            <li>
              <a class="dropdown-item" href="#" onclick="exportData('currentMonth')">
                <i class="bx bx-calendar-check me-1"></i>Export Bulan Saat Ini
              </a>
            </li>
            <li>
                ...
                <td>
                  <div>
                    <div class="fw-medium text-dark">
                      {{ $payment->invoice->customer->nama_customer ?? 'N/A' }}
                    </div>
                    <small class="text-muted">
                      {{ Str::limit($payment->invoice->customer->alamat ?? '', 30) }}
                    </small>
                  </div>
                </td>
                <td>
                  ...
                  @else
                    @if ($payment->status_id == 8)
                      <div class="d-flex justify-content-center gap-2">
                        ...
                        <a href="/kirim-ulang/{{ $payment->id }}" class="btn btn-outline-info btn-sm text-info"
                          data-bs-toggle="tooltip" data-bs-placement="bottom" title="Kirim Notifikasi Pembayaran Berhasil">
          ...
          <div>
            <nav aria-label="Page navigation">
              <ul class="pagination pagination-sm justify-content-center mb-0">
                ...
                @else
                  <li class="page-item">
                    <a class="page-link ajax-pagination-old" href="#" data-page="{{ $payments->currentPage() - 1 }}">‹
                      Previous</a>
                  </li>
                @endif
                ...
                @if ($end < $payments->lastPage())
                  ...
                  @endif
                  <li class="page-item">
                    <a class="page-link ajax-pagination-old" href="#"
                      data-page="{{ $payments->lastPage() }}">{{ $payments->lastPage() }}</a>
                  </li>
                @endif
                ...
                @if ($payments->hasMorePages())
                  <li class="page-item">
                    <a class="page-link ajax-pagination-old" href="#" data-page="{{ $payments->currentPage() + 1 }}">Next
                      ›</a>
                  </li>
                @else
                  <li class="page-item disabled">
              ...
              <div class="row">
                <div class="col-sm-6 mb-2">
                  ...
                  <div class="input-group input-group-merge">
                    ...
                    <input type="text" class="form-control"
                      value="{{ $pembayaran->invoice->customer->nama_customer ?? '-' }}" readonly>
                  </div>
                </div>
                <div class="col-sm-6 mb-2">
                  ...
                  <div class="input-group input-group-merge">
                    ...
                    <input type="text" class="form-control"
                      value="Rp {{ number_format($pembayaran->invoice->tagihan + $pembayaran->invoice->tunggakan + $pembayaran->invoice->tambahan - $pembayaran->invoice->saldo ?? 0, 0, ',', '.') }}"
                      readonly>
                  </div>
                </div>
                ...
                <div class="col-sm-12 mb-2">
                  ...
                  <div class="input-group input-group-merge">
                    ...
                    <select name="tipe_pembayaran" class="form-select" required>
                      <option value="reguler" {{ $pembayaran->tipe_pembayaran == 'reguler' ? 'selected' : '' }}>
                        Pembayaran Reguler</option>
                      <option value="diskon" {{ $pembayaran->tipe_pembayaran == 'diskon' ? 'selected' : '' }}>Pembayaran
                        Diskon</option>
                    </select>
                  </div>
                </div>
                ...
                <div class="col-sm-12 mb-2">
                  ...
                  <div class="input-group input-group-merge">
                    ...
                    <select name="metode_bayar" class="form-select" required>
                      ...
                      <option value="Transfer" {{ $pembayaran->metode_bayar == 'Transfer' ? 'selected' : '' }}>Transfer
                      </option>
                    </select>
                  </div>
                </div>
                ...
                <div class="col-sm-12 mb-2">
                  ...
                  <div class="input-group">
                    <span class="input-group-text"><i class="bx bx-money"></i></span>
                    <input type="text" name="jumlah" id="jumlah{{ $pembayaran->id }}" class="form-control"
                      value="{{ $pembayaran->jumlah_bayar ? 'Rp ' . number_format($pembayaran->jumlah_bayar, 0, ',', '.') : 'Rp 0' }}"
                      oninput="formatRupiahEdit(this, {{ $pembayaran->id }})">
                    <input type="hidden" name="jumlahRaw" id="jumlahRaw{{ $pembayaran->id }}"
                      value="{{ $pembayaran->jumlah_bayar ?? 0 }}">
                  </div>
                </div>

                <div class="col-sm-12 mb-2">
                  <label class="form-label">Keterangan Edit</label>
                  <textarea name="keterangan" class="form-control" cols="10" rows="3"
                    placeholder="Masukkan alasan edit pembayaran..."></textarea>
                </div>
              </div>
            </div>
...
@section('page-script')
  <script>
    ...
    // Apply filters automatically (renamed from applyFilters)
    function applyFiltersAutomatically() {
      ...
      // Reload page with filters for consistent pagination
      ...
      // Reload page with filters to maintain pagination consistency
      ...
      fetch(`{{ route('pembayaran.ajax') }}?${params.toString()}`, {
        ...
        .then(data => {
          if (data.success) {
            ...
            // Show notification
            const totalVisible = perPage === 'all' ? data.data.payments.length : Math.min(parseInt(perPage),
  ...
  <script>
    ...
    function exportData(filter) {
      ...
      // Ambil nilai tahun dari filter tahun jika ada
      const yearFilter = document.getElementById('yearFilter');
      const selectedYear = yearFilter ? yearFilter.value : currentYear;

      // Buat URL dengan parameter bulan dan tahun
      const url = new URL(`{{ route('pembayaran.export', '') }}/${filter}`);
      if (currentMonth) {
        url.searchParams.append('month', currentMonth);
      }
      url.searchParams.append('year', selectedYear);

      // Tambahkan parameter filter lainnya untuk konsistensi
    ...
    function exportCustomRange() {
      ...
      // Tambahkan parameter bulan dan tahun untuk konsistensi
      if (currentMonth) {
        url.searchParams.append('month', currentMonth);
      }
      url.searchParams.append('year', selectedYear);
      ...
      // Show loading notification
      showNotification('Mempersiapkan export data custom range...', 'info');
