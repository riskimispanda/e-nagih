          ...
          <div class="filter-container">
            <div class="d-flex align-items-center">
              ...
              <select class="month-filter" id="monthFilter">
                <option value="" {{ empty($currentMonth) || $currentMonth == 'all' ? 'selected' : '' }}>Semua Bulan</option>
                @for ($i = 1; $i <= 12; $i++)
                  @php
                  ...
                  <tr>
                    ...
                    <td class="text-center" data-label="Lokasi">
                      @php
                        ...
                        $url = $gps
                          ...
                          : '#';
                      @endphp
                      <a href="{{ $url }}" target="_blank" class="btn btn-icon {{ $gps ? '' : 'disabled' }}"
                        data-bs-toggle="tooltip" title="{{ $gps ? 'Lihat di Google Maps' : 'Lokasi tidak tersedia' }}">
                        <i class="bx bx-map"></i>
                      </a>
                    </td>
                    ...
                    <td data-label="Aksi">
                      <div class="d-flex justify-content-center">
                        ...
                        @endif
                        <a href="/teknisi/detail-antrian/{{ $item->id }}" class="btn btn-icon" data-bs-toggle="tooltip"
                          title="Lihat Detail">
      ...
      <div class="card">
        ...
        <div class="card-body">
                  ...
                  @if (auth()->user()->id == $item->teknisi_id or auth()->user()->roles->name == 'NOC' or auth()->user()->roles->name == 'Super Admin')
                    @if ($item->status_id == '2' or $item->status_id == '3')
                      ...
                      <tr>
                        ...
                        <td class="text-center" data-label="Lokasi">
                          <a href="{{ $item->gps }}" target="_blank" class="btn btn-icon" data-bs-toggle="tooltip"
                        ...
                        <td data-label="Aksi">
                          <div class="d-flex justify-content-center">
                            @if(auth()->user()->roles->name == 'Teknisi')
                              ...
                              <a href="/batalkan/{{ $item->id }}"
                                class="btn btn-icon btn-complete bg-label-danger me-1 @if($item->status_id == 3) disabled @endif"
                                data-bs-toggle="tooltip" title="Batalkan" data-bs-placement="bottom">
                                <i class="bx bx-x"></i>
                              </a>
                            @elseif(auth()->user()->roles->name == 'NOC')
                              <a href="/edit/antrian/{{ $item->id }}/noc" class="btn btn-icon btn-complete me-1"
          ...
          @if(auth()->user()->roles_id != 4 && $progressPerPage > 0 && method_exists($progressData, 'hasPages') && $progressData->hasPages())
            <div class="pagination-container">
              <div class="pagination-info">
                Menampilkan {{ $progressData->firstItem() }} - {{ $progressData->lastItem() }} dari
                {{ $progressData->total() }} data
              </div>
              <div>
