...
class TripayController extends Controller
{
  ...
  public function showPaymentPage($id, Request $request)
  {
    $customerId = decrypt($id);

    // Check if specific invoice_id is requested via query parameter
    $requestedInvoiceId = $request->query('invoice_id');

    if ($requestedInvoiceId) {
      // Get specific invoice by ID and verify it belongs to this customer
      $invoice = Invoice::where('id', $requestedInvoiceId)
        ->where('customer_id', $customerId)
        ->first();

      // If invoice not found or doesn't belong to customer, fallback to latest
      if (!$invoice) {
        $invoice = Invoice::where('customer_id', $customerId)->latest()->first();
      }
    } else {
      // Default: Get the latest invoice
      $invoice = Invoice::where('customer_id', $customerId)->latest()->first();
    }

    // Get all invoices for this customer (all years for history modal)
    $invoiceAll = Invoice::with(['paket', 'status'])
      ->where('customer_id', $customerId)
  ...
  public function showPaymentDetail($reference)
  {
    try {
      ...
      return redirect()->route('payment.show', 1)->with('error', 'Transaksi tidak ditemukan atau telah kedaluwarsa.');
    }
  }
   ...
   * @param int $id Encrypted Customer ID
  ...
  public function processPayment(Request $request, $id)
  {
    ...
    try {
      // ✅ FIX: Check if specific invoice_id is requested via query parameter
      $requestedInvoiceId = $request->query('invoice_id');
      ...
      if ($requestedInvoiceId) {
        ...
        // If invoice not found or doesn't belong to customer, return error
        if (!$invoice) {
          Log::warning('Invoice not found or unauthorized access', [
            'requested_invoice_id' => $requestedInvoiceId,
            'customer_id' => $customerId
          ]);
          return redirect()->back()->with('error', 'Invoice tidak ditemukan atau tidak valid.');
        }
      } else {
        // Default: Get the latest invoice
        $invoice = Invoice::where('customer_id', $customerId)->latest()->first();
