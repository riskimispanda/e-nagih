...
class KeuanganController extends Controller
{
  ...
  public function getAjaxData(Request $request)
  {
    try {
      ...
      // Add encrypted_customer_id to each invoice
      $invoicesWithEncryption = collect($invoices)->map(function ($invoice) {
        $invoiceArray = $invoice->toArray();
        $invoiceArray['encrypted_customer_id'] = encrypt($invoice->customer_id);
        return $invoiceArray;
      })->toArray();
      ...
      return response()->json([
        'success' => true,
        'data' => [
          'invoices' => $invoicesWithEncryption,
  ...
  public function requestPembayaran(Request $request, $id)
  {
    ...
    try {
      ...
      $keteranganArr = [];
      if ($bayarTagihan > 0)
        $keteranganArr[] = "Tagihan Langganan";
      if ($bayarTambahan > 0)
        $keteranganArr[] = "Biaya Tambahan";
      if ($bayarTunggakan > 0)
        $keteranganArr[] = "Tunggakan";
      if ($saldoTerpakai > 0)
        $keteranganArr[] = "menggunakan saldo";
      if ($saldoBaru > 0)
        $keteranganArr[] = "menyisakan saldo";
  ...
  public function agen(Request $request)
  {
    ...
    $agenQuery = User::where('roles_id', 6)
      ->withCount([
        'customer as total_customer' => function ($query) {
  ...
  public function searchAgen(Request $request)
  {
    ...
    $query = User::where('roles_id', 6)->withCount([
      'customer' => function ($query) use ($currentMonth) {
