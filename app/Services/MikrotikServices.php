...
class MikrotikServices
{
  ...
  public static function getFirewallRules(Router $router)
  {
    ...
    foreach ($apiFilters as $f) {
      if (isset($f['src-address-list']))
        $lists[] = $f['src-address-list'];
      if (isset($f['dst-address-list']))
        $lists[] = $f['dst-address-list'];
    }
  ...
  public static function changeUserProfileSingle(Client $client, $usersecret, $profileName = 'ISOLIREBILLING')
  {
    try {
      // ✅ VALIDASI INPUT
      if (empty($usersecret) || trim($usersecret) === '') {
        Log::error('changeUserProfileSingle: usersecret kosong atau invalid', [
          'usersecret' => $usersecret,
          'profile' => $profileName
        ]);
        return false;
      ...
      if (empty($users)) {
        Log::warning('changeUserProfileSingle: User tidak ditemukan', [
          'usersecret' => $usersecret,
          'profile' => $profileName
        ]);
        return false;
      ...
      // ✅ VALIDASI MULTIPLE USERS (SAFETY CHECK!) - Return FALSE bukan string
      if (count($users) > 1) {
        Log::error('⚠️ CRITICAL: Multiple users found untuk usersecret. Operasi dibatalkan untuk keamanan!', [
          'usersecret' => $usersecret,
          'count' => count($users),
          'profile' => $profileName
        ]);
      ...
      $user = $users[0];
      if (!isset($user['name']) || $user['name'] !== $usersecret) {
        Log::error('changeUserProfileSingle: Name tidak exact match', [
          'expected' => $usersecret,
          'got' => $user['name'] ?? 'null',
          'profile' => $profileName
        ]);
      ...
      Log::info('✅ changeUserProfileSingle Success', [
        'usersecret' => $usersecret,
        'user_id' => $user['.id'],
        'profile' => $profileName
      ]);
      ...
      Log::error('❌ changeUserProfileSingle error', [
        'usersecret' => $usersecret,
        'profile' => $profileName,
        'error' => $e->getMessage()
      ]);
  ...
  public static function getPPPSecret(Client $client)
  {
    try {
      ...
      $query->where('name', '0110171120235.Yunus-Jetis@megaroute.net.id');
      return $client->query($query)->read();
    } catch (\Exception $e) {
  ...
  public static function editUserSecret(Client $client, string $id, array $data)
  {
    ...
    if (isset($data['password']))
      $query->equal('password', $data['password']);
    if (isset($data['remoteAddress']))
      $query->equal('remote-address', $data['remoteAddress']);
    if (isset($data['localAddress']))
      $query->equal('local-address', $data['localAddress']);
  ...
  public static function removeActiveConnections(Client $client, $usersecret)
  {
    try {
      // ✅ VALIDASI INPUT
      if (empty($usersecret) || trim($usersecret) === '') {
        Log::error('removeActiveConnections: usersecret kosong atau invalid', [
          'usersecret' => $usersecret
        ]);
        return false;
      }
      ...
      if (empty($activeConnections)) {
        Log::info('removeActiveConnections: Tidak ada koneksi aktif', [
          'usersecret' => $usersecret
        ]);
        return false;
      }

      // ✅ VALIDASI EXACT MATCH & SAFETY CHECK
      $validConnections = [];
      foreach ($activeConnections as $connection) {
        if (isset($connection['name']) && $connection['name'] === $usersecret) {
          $validConnections[] = $connection;
        } else {
          Log::warning('removeActiveConnections: Name mismatch detected', [
            'expected' => $usersecret,
            'got' => $connection['name'] ?? 'null',
            'interface' => $connection['interface'] ?? 'unknown'
          ]);
        }
      }

      if (empty($validConnections)) {
        Log::warning('removeActiveConnections: Tidak ada koneksi yang exact match', [
          'usersecret' => $usersecret,
          'total_found' => count($activeConnections)
        ]);
        return false;
      ...
      // ✅ SAFETY: Jangan disconnect lebih dari 5 koneksi sekaligus
      if (count($validConnections) > 5) {
        Log::error('⚠️ CRITICAL: Lebih dari 5 koneksi untuk 1 user. Operasi dibatalkan untuk keamanan!', [
          'usersecret' => $usersecret,
          'connection_count' => count($validConnections)
        ]);
        return false;
      }

      // ✅ REMOVE HANYA KONEKSI YANG VALID
      $disconnectedCount = 0;
      $failedCount = 0;
      ...
      foreach ($validConnections as $connection) {
        try {
          ...
          $disconnectedCount++;
          Log::info('✅ removeActiveConnections: Koneksi berhasil diputus', [
            'usersecret' => $usersecret,
          ...
          $failedCount++;
          Log::error('❌ removeActiveConnections: Gagal memutus koneksi', [
            'usersecret' => $usersecret,
            'connection_id' => $connection['.id'],
            'error' => $e->getMessage()
          ]);
      ...
      Log::info('✅ removeActiveConnections: Selesai', [
        'usersecret' => $usersecret,
        'disconnected' => $disconnectedCount,
        'failed' => $failedCount,
        'total_valid' => count($validConnections)
      ]);

      return $disconnectedCount > 0;

    } catch (Exception $e) {
      Log::error('❌ removeActiveConnections: Exception', [
        'usersecret' => $usersecret,
        'error' => $e->getMessage()
      ]);
      return false;
